#include<bits/stdc++.h>
using namespace std;

#define roundNumber 10
#define paddingCharacter 'X'

unsigned char key[]="madeit6teenbytes";
unsigned char modifiedKey[176];
int extendedLen=16;
unsigned char sBox[256] =
{
	
} ;


void substitutionWord(unsigned char* input){
	for(int i=0;i<=3;i++)
		input[i]=sBox[input[i]];
}

void rotationOfword(unsigned char* input){
	unsigned char temp=input[[0];
	
	for(int i=0;i<=2;i++)
	input[i]=input[i+1];
	
	input[3]=temp;
}

void expansionKey(unsigned char *originalKey){

	int idx;
	for(idx=0;idx<=15;idx++)
		modifiedKey[idx]=originalKey[idx];
		
	while(idx<=175){
	
		unsigned char prevWord[[4];
		for(int i=0;i<4;i++)
			prevWord[i]=modifiedKey[idx+1-4];
		
		if(idx%16==0){
			rotationOfWord(prevWord);
			substitutionWord(prevWord);
			prevWord[0]^=roundConstant[(idx/16)-1];	
		}
		
		for(int k=0;k<4;k++)
		modifiedey[idx+k]=prevWord[k]^modifiedKey[idx-16+k];
		idx+=4;
	}	
		
}

addRoundKey(unsigned char* state,int roundNum){
	for(int i=0;i<16;i++)
	state[i]^=modifiedKey[roundNum*16+i];
}


encryption(unsigned char* message){

	expansionKey(key);
	addRoundKey(message,0);
	
	for(int i=1;i<==roundNumber;i++){
		subBytes(message);;
		cyclicLeftShiftRow(message);
		
		if(i!=roundNumber)
			//mixColumn(message);
			
			//addRoundKey(message,i);
	}	
}
getEncryptedText(string input){
		
	int len=input.length();
	char inputText[len];
	
	for(int i=0;i<len;i++)
		inputText[i]=input[i];
	
	int txtLen=sizeof(inputText);
	
	if(txtLen%16)!=0)
		extendedLen=txtLen+(16-(txtLen%16));
	else
		extendedLen=txtLen;
	
	unsigned char encryptedTxt[extendedLen+1],man[exytendedLen+1];
	
	for(int i=0;i<extendedLen;i++){
		if(i<txtLen)
		encryptedTxt[i]=inputText[i];
		else
		encryptedTxt[i]=paddingCharacter;
		if(i+1==extendedLen)
		encryptedTxt[i+1]='\0';
	}	
	
	for(int i=0;i<xtendedLen;i+=16){
		unsigned char temp[16];
		for(int j=0;j<16;j++)
			temp[j]=encryptedTxt[i+j];
			encryption(temp);
			for(int j=0;j<16;j++)
				encryptedTxt[i+j]=temp[j];
	}
	mess.resize(17);
	for(inti=0;i<=extendedLen;i++){
		mess[i]=encryptedTxt[i];
		man[[i]=mess[i];
	}
	
	returrn mess;
}
int main(){

	string input,encrypted;
	freopen("AESinput.txt","r",stdin);
	getline(cin,input);	
	
	encrypted=getEncryptedText(input);
}
